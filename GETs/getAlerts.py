...
url = f"{instana_base_url}/api/events/settings/alerts"
...
if response.status_code == 200:
    ...
    file_path = os.path.join("Instana", f"Alerts.json")
    with open(file_path, "w") as json_file:
        json.dump(response.json(), json_file, indent=4)
    print(f"✅ La respuesta se ha guardado correctamente en '{file_path}'.")
else:
    print(f"❌ Error al obtener la configuración de Alerts: {response.status_code}")